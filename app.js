...
const { Search, MapPin, Clock, Euro, Building, User, Plus, X, Filter } = lucide;
...
const Jobflix = () => {
    ...
    const initialJobs = [
      {
        ...
        type: 'Voltijd',
        categorie: 'IT',
        beschrijving: 'Wij zoeken een ervaren Frontend Developer om ons team te versterken. Je werkt met React, TypeScript en moderne tools.',
        vereisten: ['3+ jaar ervaring met React', 'TypeScript kennis', 'CSS/SCSS ervaring'],
      ...
      },
      {
        ...
        type: 'Voltijd',
        categorie: 'Marketing',
        beschrijving: 'Ervaren Marketing Manager gezocht voor het leiden van marketingcampagnes en teammanagement.',
        vereisten: ['5+ jaar marketing ervaring', 'Teamleiding ervaring', 'Digital marketing kennis'],
        geplaatst: '1 dag geleden'
      },
      {
        id: 3,
        titel: 'UX Designer',
        bedrijf: 'Design Studio',
        locatie: 'Utrecht',
        salaris: '€3.000 - €4.500',
        type: 'Parttime',
        categorie: 'Design',
        beschrijving: 'Creatieve UX Designer om gebruiksvriendelijke interfaces te ontwerpen voor diverse klanten.',
        vereisten: ['Portfolio met UX projecten', 'Figma/Sketch ervaring', 'User research ervaring'],
  ...
  useEffect(() => {
    ...
    if (locationFilter) {
      filtered = filtered.filter(job => 
        job.locatie.toLowerCase().includes(locationFilter.toLowerCase())
      );
    }
    
    if (categoryFilter) {
      filtered = filtered.filter(job => job.categorie === categoryFilter);
    }
  ...
  const JobDetail = ({ job }) => React.createElement('div', { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" },
    React.createElement('div', { className: "bg-white rounded-lg max-w-2xl w-full max-h-90vh overflow-y-auto" },
      React.createElement('div', { className: "p-6" },
        ...
        React.createElement('div', { className: "grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" },
          React.createElement('div', { className: "flex items-center gap-2 text-gray-600" },
            ...
            React.createElement('span', { className: "text-sm" }, job.locatie)
          ),
          React.createElement('div', { className: "flex items-center gap-2 text-gray-600" },
            React.createElement(Euro, { className: "w-4 h-4" }),
            React.createElement('span', { className: "text-sm" }, job.salaris)
          ),
          React.createElement('div', { className: "flex items-center gap-2 text-gray-600" },
            React.createElement(Clock, { className: "w-4 h-4" }),
            React.createElement('span', { className: "text-sm" }, job.type)
          ),
          React.createElement('div', { className: "flex items-center gap-2 text-gray-600" },
            React.createElement(Filter, { className: "w-4 h-4" }),
            React.createElement('span', { className: "text-sm" }, job.categorie)
          )
        ),
        ...
        job.vereisten && job.vereisten.length > 0 && React.createElement('div', { className: "mb-6" },
          ...
          React.createElement('ul', { className: "space-y-2" },
            ...job.vereisten.map((req, index) => React.createElement('li', { key: index, className: "flex items-start gap-2 text-gray-600" },
        ...
        React.createElement('div', { className: "flex gap-3 pt-4 border-t" },
          React.createElement('button', { className: "flex-1 bg-red-600 text-white py-3 px-4 rounded-md hover:bg-red-700 transition-colors font-medium" }, "Solliciteren"),
          React.createElement('button', { className: "flex-1 bg-gray-100 text-gray-700 py-3 px-4 rounded-md hover:bg-gray-200 transition-colors font-medium" }, "Opslaan")
        )
      )
    )
  );
  ...
  return React.createElement('div', { className: "min-h-screen bg-gray-50" },
        ...
        React.createElement('div', { className: "flex justify-between items-center h-16" },
          ...
          React.createElement('nav', { className: "flex space-x-8" },
            React.createElement('button', {
              ...
              React.createElement(Search, { className: "w-4 h-4 inline mr-1" }),
              "Zoeken"
            ),
            React.createElement('button', {
    ...
    React.createElement('main', { className: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" },
      activeTab === 'zoeken' && React.createElement('div', {},
        ...
        React.createElement('div', { className: "bg-white rounded-lg shadow-sm p-6 mb-8" },
          React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-3 gap-4" },
            React.createElement('div', { className: "relative" },
              React.createElement(Search, { className: "absolute left-3 top-3 w-4 h-4 text-gray-400" }),
              React.createElement('input', {
                type: "text",
            ...
            React.createElement('div', { className: "relative" },
              React.createElement(MapPin, { className: "absolute left-3 top-3 w-4 h-4 text-gray-400" }),
              React.createElement('input', {
                type: "text",
                placeholder: "Locatie...",
            ...
            React.createElement('div', { className: "relative" },
              React.createElement(Filter, { className: "absolute left-3 top-3 w-4 h-4 text-gray-400" }),
              React.createElement('select', {
                value: categoryFilter,
                onChange: (e) => setCategoryFilter(e.target.value),
                className: "w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 appearance-none"
              },
        ...
        // Jobs List
        React.createElement('div', { className: "space-y-4" },
          ...filteredJobs.map(job => React.createElement('div', { 
            key: job.id, 
            ...
            React.createElement('div', { className: "flex justify-between items-center" },
              ...
              React.createElement('div', { className: "flex gap-2" },
                React.createElement('button', { className: "px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors text-sm" }, "Solliciteren"),
                React.createElement('button', { className: "px-4 py-2 bg-gray-100 text-gray-600 rounded-md hover:bg-gray-200 transition-colors text-sm" }, "Opslaan")
              )
            )
          )),
          ...
          filteredJobs.length === 0 && React.createElement('div', { className: "text-center py-12" },
            React.createElement('div', { className: "text-gray-400 mb-4" },
              React.createElement(Search, { className: "w-12 h-12 mx-auto" })
            ),
        ...
        React.createElement('div', { className: "bg-white rounded-lg shadow-sm p-8" },
          ...
          React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-3 gap-8 mb-8" },
            React.createElement('div', { className: "text-center" },
              React.createElement('div', { className: "bg-red-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" },
                React.createElement(User, { className: "w-8 h-8 text-red-600" })
              ),
            ...
            React.createElement('div', { className: "text-center" },
              React.createElement('div', { className: "bg-red-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" },
                React.createElement(Search, { className: "w-8 h-8 text-red-600" })
              ),
            ...
            React.createElement('div', { className: "text-center" },
              React.createElement('div', { className: "bg-red-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" },
                React.createElement(Clock, { className: "w-8 h-8 text-red-600" })
              ),
    ...
    // Modals
    showJobForm && React.createElement(JobForm),
    selectedJob && React.createElement(JobDetail, { job: selectedJob })
  );
};

ReactDOM.render(React.createElement(Jobflix), document.getElementById('root'));